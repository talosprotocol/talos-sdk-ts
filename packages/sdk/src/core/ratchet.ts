import { KeyPairX25519, generateKeyPair, getSharedSecret, getPublicKey } from '../crypto/x25519.js';
...
export class RatchetSession {
    ...
    public encrypt(plaintext: Uint8Array, explicitNonce?: Uint8Array): string {
        if (this.state.CKs === null) {
            ...
            const [rk, cks] = this.kdfRk(this.state.RK, getSharedSecret(this.state.DHs.privateKey, this.state.DHr));
            this.state.RK = rk;
            this.state.CKs = cks;
        }
    ...
    private dhRatchet(peer_dh: Uint8Array) {
        ...
        const [rk1, ckr] = this.kdfRk(this.state.RK, getSharedSecret(this.state.DHs.privateKey, this.state.DHr));
        this.state.RK = rk1;
        this.state.CKr = ckr;
        ...
        const [rk2, cks] = this.kdfRk(this.state.RK, getSharedSecret(this.state.DHs.privateKey, this.state.DHr));
        this.state.RK = rk2;
        this.state.CKs = cks;
    }

    private trySkippedMessageKeys(peer_dh: Uint8Array, n: number): Uint8Array | null {
    ...
    private skipMessageKeys(until: number) {
        ...
        if (this.state.CKr !== null) {
            while (this.state.Nr < until) {
                const [ck, mk] = this.kdfCk(this.state.CKr);
                this.state.CKr = ck;
