...
const getVectorsRoot = () => {
  ...
  return path.join(curr, "test_vectors");
};
const VECTORS_ROOT = getVectorsRoot();
...
describe("Phase 4.1 Vector Compliance", () => {
  ...
  describe("Positive Vectors", () => {
    it("Capability Canonicalization (Content)", async () => {
      ...
      const expectedBytes = readBytes(
        "positive/capability_content_canonical.bytes",
      );
    ...
    it("Capability Token Canonicalization (With Sig)", async () => {
      const cap = readJson("positive/capability.json");
      const expectedBytes = readBytes(
        "positive/capability_token_canonical.bytes",
      );
  ...
  describe("Negative Vectors", () => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const checkNegative = async (
      filename: string,
      checkFn: (input: any) => Promise<boolean>,
    ) => {
    ...
    it("wrong_sig matches expected denial", async () => {
      const result = await checkNegative("wrong_sig.json", (input) =>
        verifyCapability(input, keyPair.publicKey),
      );
    ...
    it("non_canonical_json_order matches expected denial", async () => {
      ...
      const result = await checkNegative(
        "non_canonical_json_order.json",
        (input) => {
    ...
    it("missing_request_binding should fail frame strictness or signature", async () => {
      ...
      const result = await checkNegative(
        "missing_request_binding.json",
        (input) => verifyFrame(input, keyPair.publicKey),
      );
      expect(result).toBe(false);
    });
  });
